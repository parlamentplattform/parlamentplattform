...
def test_beendetes_mandat_oeffnet_den_bereich_nicht(client):
    m, mandat = mandatar(client, beendet=timezone.localdate())
...
def test_monatsbericht_nur_fuer_faellige_monate(client, monkeypatch):
    ...
    # kein zweiter Bericht für denselben Monat — der Monat ist nicht mehr fällig
    html = client.post(AKTION, {"aktion": "monatsbericht", "mandat": mandat.pk, "monat": "2026-10-01", "text": "Nachtrag"}, follow=True).content.decode()
    assert mandat.berichte.count() == 1 and "fälligen Monat" in html
...
def test_laufende_mandatsfrage_laesst_keinen_erfundenen_beschluss_zu(client, ordnung):  # noqa: F811
    ...
    eintrag = mandat.rechenschaft.get()
    assert eintrag.antrag is None and eintrag.beschluss_plattform == "keiner"
    assert "läuft noch" in html
