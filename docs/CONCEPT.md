...
| F-71 | **Mandatar-Steuerung (E-2.5):** öffentliche Mandatare-Seite (Foto, aktuelle Aufgaben, laufende Entscheidungsprozesse samt Fristen) je Mandatsträger; Rolle „Mandatar" mit Instant-Reports und den daraus entstehenden, vom Mandatar betreuten Abstimmungen; Informationspflicht aus der Mandatsvereinbarung; Ergebnisse als Beschlusslage und Richtschnur, freies Mandat unberührt. *Stand 0.46 (Erstfassung umgesetzt): Rolle „Mandatar" im Code — abgeleitet vom offenen Mandat, kein Rollendatensatz; eigener Bereich `/mandatare/mein/` mit Instant-Report (Frist als Zeitpunkt, Sitzungstag-Kennzeichen) und daraus der Mandatsfrage als eigener Antragsart, die ohne Unterstützungs- und Beratungsphase sofort in der Abstimmung steht (§ 7 Abs 9; Dauer aus dem Register, nie unter § 5 Abs 3 lit d, beim Eröffnen eingefroren); Sammelbericht binnen satzungsfester sieben Tage nach dem Sitzungstag und Monatsbericht mit Karenz aus dem Register (§ 7 Abs 3 lit b); Lichtbild und Vorstellung pflegt der Mandatar selbst; Wahlvorschlag einer beendeten Kandidatur als Markdown. **Stand 0.48 (Vertrauensfrage, § 7 Abs 10):** eigene Antragsart `vertrauensfrage` — jedes Mitglied stellt sie auf der Seite des Mandatars mit mindestens einem Anlass (Rechenschaftseintrag mit Abweichung oder Ausstand über 30 Tage; Formerfordernis, keine inhaltliche Bewertung); unterstützen kann, wer am Einbringungstag für Personenwahlen stimmberechtigt ist, Schwelle fünf Prozent dieser Mitglieder am Einbringungstag (mit dem Antrag veröffentlicht); keine Beratungsphase, Abstimmung frühestens am siebten Tag nach Einbringung, spätestens am dritten nach Erreichen der Schwelle; Stellungnahme des Mandatars append-only neben dem Antrag; Sperren nach lit g als Hinweis an den Integritätsrat, der sie binnen drei Tagen durch Beschluss feststellt — die Software weist nichts ab; Ergebnis „verloren“ (angenommen) / „gewonnen“; Wirkungen nach lit f in zwei Stufen (Ruhen der Ratsrollen und Kandidatursperre sofort; Ende der Rollen nach der Anfechtungsfrist, Ende der Vertretung 30 Tage nach dem Ergebnis — nie ein Eingriff in das staatliche Mandat, nichts setzt `Mandat.beendet`); Rückgabezusage als Pflichtangabe bei der Bewerbung; Bestätigungsantrag frühestens sechs Monate später; Anfechtung und Entscheidung des Parteischiedsgerichts als Vermerke der Verwaltung; öffentlich `/vertrauensfragen/` und Abschnitt „Vertrauen“ auf der Mandatar-Seite. Der regionale Weg (lit c zweiter Satz) wartet auf Gliederungen (§ 14 Abs 4). Offen: Vollzugsbericht durch den Mandatar, Mandatsvereinbarung als Dokument, Anrufungsablauf beim Parteischiedsgericht.* | M | § 7 Abs 2, 3, 5, 9, 10 |
...
| F-24 | **Rechenschaftsregister:** je Parlamentsabstimmung Beschluss der Plattform, tatsächliche Stimme, Begründung des Mandatars, Zeitstempel ≤ 7 Tage, dauerhaft, filterbar. *Stand 0.46 (Erstfassung umgesetzt): Modell `Rechenschaft` (App mandatare) — Gegenstand, Sitzungstag, Beschluss der Plattform (aus dem verknüpften Antrag abgeleitet), Stimme im Vertretungskörper, Begründung, Zeitstempel; öffentlich `/rechenschaft/` (Filter nach Ebene), je Mandatar mit Ausständen („ausständig seit …", ohne Sanktion), `/rechenschaft.json`; die sieben Tage sind satzungsfest in `plattform_core/rechenschaft.py`, keine Stellgröße; Abweichung vom Plattformbeschluss wird nebeneinander gezeigt, nicht bewertet. **Stand 0.48:** Einträge mit Abweichung und ausgewiesene Ausstände über 30 Tage stehen als Anlässe einer Vertrauensfrage zur Auswahl (§ 7 Abs 10 lit b); das Register weist Ergebnisse von Vertrauensfragen, Rückgabeersuchen und -frist, Rückgabezusage und die Ergänzung der Mandatsvereinbarung als Sachverhalt aus (lit e, lit f Z 4, lit j) — ohne Wertung; `/rechenschaft.json` führt `vertrauensfragen`. Offen: weitere Filter (Zeitraum, Stimmverhalten).* | S | § 7 Abs 5, Abs 10 |
...
| Rechenschaftsregister | Eigenentwicklung nötig | Eigenentwicklung nötig | eingeplant (gebaut mit 0.46, F-24) |
