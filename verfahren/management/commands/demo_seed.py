...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ordnung, _ = Verfahrensordnung.objects.get_or_create(
            policy_id="sachantrag-standard", version=1,
            defaults={"regeln": {
                "id": "sachantrag-standard", "version": 1,
                "unterstuetzung_schwelle": 3, "unterstuetzung_frist_tage": 14,
        ...
        if not Antrag.objects.exists():
            a1 = antrag_einbringen(
                leute[0], "Sitzungsprotokolle binnen 48 Stunden veröffentlichen",
                "Die DDÖ veröffentlicht Protokolle aller Ratssitzungen binnen 48 Stunden auf der Plattform.",
                "Transparenz ist unser Kerngeschäft — sie beginnt bei uns selbst.", ordnung,
            )
            a2 = antrag_einbringen(
                leute[1], "Testlauf: monatlicher öffentlicher Entwicklungsbericht",
            ...
            a3 = antrag_einbringen(
                leute[2], "Abgeschlossenes Beispiel: Namenskonvention des Prototyps",
                "Der Prototyp führt den Namen ParlamentPlattform.", "", ordnung,
            )
            for m in leute[:4]:
            ...
            a3.fortschreiben()  # -> Abstimmung
            a3.stimmberechtigte_anzahl = Mitglied.objects.count()
            a3.save(update_fields=["stimmberechtigte_anzahl"])
